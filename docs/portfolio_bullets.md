...
- **Scoped the forward work to attack my own #1 disclosed risk:** a ratified post-M5 roadmap (ADR-0033/0034) that (a) **replaces the AprilTag with a markerless seeker** — classical detect-then-track plus a lightweight neural detector feeding the *same* bearing interface, so the guidance is unchanged — (b) A/Bs an **EKF vs. the alpha-beta tracker** with a *pre-registered null on miss* (the miss is kinematic, not estimator-limited — ADR-0023), and (c) a **covariance-gated mid-course sensor-fusion capstone**, all validated against a **~$230 Raspberry Pi 5 perception bench** that measures the real sim-to-real detection gap before any airframe spend. *(ADR-0012/0033/0034)*
...
- **A concrete plan to un-fake the faked parts.** The disclosed risks aren't left as caveats — there's a ratified next phase (ADR-0033/0034): kill the AprilTag with a markerless seeker, A/B an EKF against the current tracker (with the honest prediction that it won't move the kinematically-limited miss), fuse the ground and onboard sensors mid-course, and measure the whole perception stack on a ~$230 Raspberry Pi 5 bench before buying any hardware. Attacking your own weakest disclosed points is the maturity signal.

---

## 4. 30-second verbal pitch

> "It's a simulated counter-drone interceptor — a quadcopter that catches another drone using only its own camera. It detects a marker on the target, turns that into a bearing, and steers with proportional navigation, the same guidance law missiles use. The headline is a two-stage engagement: a ground sensor cues a fast dash, then hands off to camera-only for the terminal phase — so if the datalink gets jammed, the drone finishes on its own. The interesting part wasn't just that pro-nav beat pursuit by about 5-to-7×; it's that when I pushed to fast targets and the intercepts stopped tightening, I ran a 41-flight forensic batch and *proved* the miss was kinematic — locked in before the terminal phase — not a perception problem. So instead of chasing a better camera, I fixed the mid-course track and got it back under a meter and a half. And I'm upfront that the marker and the ground cue are stand-ins — the real seeker is the honest next risk, and I've scoped exactly how I'd attack it: kill the marker with a real seeker, A/B a Kalman filter I already expect won't move the miss, and measure it all on a cheap Pi bench before I ever buy hardware."
