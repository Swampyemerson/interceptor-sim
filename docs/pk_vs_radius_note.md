...
seeker, realistic degraded cue. Per this repo's convention the `logs/` CSV is
**gitignored (regenerable), not committed** — it is present in the working
tree, is regenerated by the ADR-0036 `mc_batch.sh` configuration, and the
numbers below cross-check exactly against the committed plot
