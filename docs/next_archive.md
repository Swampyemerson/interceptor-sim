# NEXT.md ARCHIVE — the full work queue as it stood 2026-07-26, verbatim

> This is the COMPLETE pre-cleanup `NEXT.md` (883 lines), preserved unedited because this
> project's rule is that superseded material stays recoverable, never deleted. It contains the
> layered session history from 2026-07-10 through 2026-07-25: the Phase-2 plan, the ROUND-5
> correction, the r2l arc, the real-build pivot, the REVIEW-2 stream, and the 2026-07-25
> what's-left push. The LIVE queue is now `NEXT.md` (short) and the GitHub board
> (https://github.com/users/Swampyemerson/projects/1), with the canonical state in
> `docs/project_state.json`.
>
> Nothing here was edited. If you need the reasoning behind a decision, prefer `docs/decisions.md`
> (the ADRs) — this file is the working-queue history, not the decision record.
...
> **✅ 2026-07-25 WHAT'S-LEFT AUDIT — WORKED TO ZERO** (audit: [`docs/audit_2026-07-25_whats_left.md`], 8-agent workflow, 106 items). **DONE this push** (ADRs 0084-0089, `run_tests.sh` green, contract+dashboard+artifact re-rendered): ram/kill radius RATIFIED 0.35 m from the ordered 5-inch geometry (ADR-0084, arithmetic in the ADR; swept every Pk surface + aim budget); the frozen_vworld zero-command sim terminal + its blind auditor + wall-clock scheduling + inert-flag guards FIXED (coded-dash camera-arm verdicts → PENDING RE-FLY); all 6 field-day P0s closed (decimate / mission geometry / capture-matrix / range-truth / desk-fps whitelist / print artifacts); FAILSAFE 7 + kill-day protocol + regulatory-capture template written; CI GREEN (was 73/73 red); Tier-1 fully ordered. **REMAINING (small, tracked below + in the contract):** ~~the m4 negative-r_hat latch follow-up~~ **FIXED `86d60b3` 2026-07-25 eve** (seeker_loop broken-track parity: negative r_hat can no longer ARM the coast; truth-table + replay drills); the coded-dash camera-arm re-fly is **✅ FLOWN + SCORED** (2026-07-25 eve, all 40 flights on the fully-fixed terminal; full table `docs/flight_plan_candidates.md` RESULTS: **the zero-command downgrades RESOLVE to a CONFIRMATION** — no camera earn at 0°/5°/15° aim error (3/8, 1/8, 4/8 vs the pre-registered ≥6/8), with one honest softening: at correct aim the camera is now ~**parity** with the dash (0.895/0.750 vs the 0.75 floor), not a liability; the 190 ms LOS-lag comp is **confirmed negative**; audit (e) 0/674 zero-command ticks proves the terminal steered; pre-fix logs archived `logs/prefix_zerocmd_terminal_20260725/`); builder to fill `docs/regulatory_site_capture.md` + confirm the 2nd Pocket TX (ADR-0089) isn't already ordered; ~~the T25 demo re-render (ram-radius caption; ADR-0066 claims-scrub)~~ **✅ RE-RENDERED 2026-07-25 late** (`demo_out/t25/t25_demo.mp4`, same 7-shot cut, captions verified in-pixels: net-class R 1.5 m named as such + ram 0.35 m at both sites, comms-denied HELD wording, the hardened n=72 CI claim added, the billboard-stand-in disclosure added, tilt card carries the ADR-0068 no-adoption note; every ADR-0066 tracker row checked — reproduction recipe `scripts/video/t25_render_plan.md`; **found+fixed en route:** ADR-0064's quoted median was median_high not the midpoint (1.784→1.752, ADR corrected) and `kill_mechanism.md` §7 still recommended the retired 0.5 m (swept). **⚠️ still-stale demo assets (builder call):** `demo_out/interceptor_onboard.{mp4,gif}` + tracked stills `docs/images/demo_onboard_final.png`/`demo_chase_final.png` bake the old un-named "1.5 m lethal radius" caption and CANNOT be re-rendered — their hero CSV was rotated off disk; re-flying = a re-shoot with a different CPA. Generator fixed for any future shoot; do NOT publish those three as ram kills — noted in `demo_out/README.md`); ~~the incidence-RATE curve producer (needs per-frame attitude)~~ **BUILT 2026-07-25 eve** — `range_truth_join.py --incidence --mount-index-deg` reconstructs a per-frame `incidence_deg` from the target log's ATT + the surveyed tripod + the pass card's index angle (geometry `scripts/seeker/placard_incidence.py`), so §4.2b's decode-RATE-vs-(range × incidence) row is now reachable instead of the decodes-only annotation; producer→consumer contract test `tests/test_incidence_rate_curve_contract.py` (13 tests). **🔨 PHYSICAL BUILD STARTED 2026-07-25 eve:** target frame carbon + Pixhawk 6C Mini + Pi 5 in hand; frame skeleton assembly begun (mechanical front half of tgt-03 — dry-fit only; no solder/power until the smoke-stopper consumables land). The physical arc (tripod → build+bench → kill ladder) is unblocked and now MOVING.

> **🗺️ Prior banner (superseded by the ✅ above):** the audit had flagged 6 desk-fixable tripod P0s, the frozen_vworld defect, CI 73/73 red, a 4-commit-stale contract, and the ram-radius ratification — all now actioned.
...
**DONE this session (all committed + pushed — the full arc):**
- [x] **Anti-mirage decomposition of the intercept** (many commits `cdf695b`…`1cfb909`, doc `docs/flight_plan_candidates.md` RESULTS/SYNTHESIS). Bdash settled + CORRECTED (the "closing-speed" verdict was confounded by a mis-sized aim). Then G20dash/G20/CG/GL/AAL/EAL decomposed it fully — **see SETTLED CONCLUSIONS below.**
- [x] **Field bring-up kit** (`334d0ca`) — `scripts/field/` + `docs/field_bringup.md`; `bringup.sh` exit 0 no-hardware, `selftest.sh` 22 checks. Camera is MIPI-CSI to the Pi → `--pi/--local/--replay`; FC bench check arming-impossible (AST-audited).
- [x] **Flight-plan + launch-mech docs** (`334d0ca`) — decomposition sweep spec + `docs/launch_mechanism_plan.md` (ground-takeoff→standby-hover→human-trigger→coded-dash, camera bearing latch on the critical path), `scripts/experiments/flight_plans/` harness + `dash_cpa_model.py`.
- [x] **Detection/tracking method stack** (`8e84ac6`) + **THREE defects fixed:** (a) hardware default-weights → real-data n-mono not the blind quad_v2 (`c3bd0d9`); (b) `seeker_loop` connect() hang, SITL-verified (`fac9f10`); (c) tripod money-gate → conservative run-length model, **ADR-0079** (`db03876`).
- [x] **Aspect-bias compensation BUILT** (`e0bedf4`, `--terminal-bearing-bias`/`--terminal-los-lag` + `measure_aspect_bias.py`) → **validation NULL** (constant regime-confounded; lag GL null 1.44 vs 1.32). The ADR-0056 terminal wall does NOT yield to a simple constant/lag.
- [x] **Accel-aware collision-lead BUILT + SIM-VALIDATED — ADR-0080** (`6adb4d7`, `--dash-accel-aware-lead`). Auto-derives the aim (+21.3° ≈ the confirmed +20°): AAL-123 = hand-tuned (0.71), AAL-777 generalizes (0.57, disjoint), honesty-clean. **The dominant lever is now automated + carries to hardware, no per-config aim tuning.**

**🏁 SETTLED CONCLUSIONS — the intercept is fully characterized (the session's headline):**
1. **AIM is the dominant lever.** Correcting the crossing-bias (the collision-lead assumed constant speed; the dash accelerates from rest) took the open-loop dash-only miss **1.37 → 0.75 m** (G20dash), and the accel-aware lead now **derives** that aim automatically (ADR-0080, validated on 2 seeds).
2. **The camera is an aim-ERROR corrector, gated by the aspect bias.** It recovers a badly-aimed dash (l2r 2.05→0.68) but at correct aim its ADR-0056 aspect bias makes it a slight liability (G20 1.02–1.32 ≥ dash 0.75), worst on r2l. Its real value = defending imperfect real-world aim (the sim's perfect-aim dash understates it).
3. **Framing (loft+wedge) HURTS at correct aim** (CG 1.92 > level 1.32) — net-negative at 9 m/s.
4. **Terminal-precision compensations are NULLs** (bias-constant regime-confounded; LOS-lag GL 1.44 vs 1.32) — the terminal wall doesn't yield to simple corrections (cf. the ADR-0071 subpixel null).
- **⇒ Path to smallest miss:** correct aim (DONE, auto) + **real-data outdoor detection** (the acquisition wall — a HARDWARE/FIELD task, not sim). Keep the terminal simple + level. `flight/fov_guidance.py` stays parked (Phase B FOV-hold was the wrong lever).
