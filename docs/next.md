...
- **⚠️ THE PUBLISHED DASHBOARD ARTIFACT IS STALE AND ONLY THE BUILDER'S SESSION CAN FIX IT
  CHEAPLY (2026-09-10).** `docs/dashboard.html` and `docs/mbse.html` in the repo ARE current
  and committed. The hosted mirror at the `artifact_url` in `docs/project_state.json` still
  serves **rev `d3004f1`, `state.updated=2026-08-29`** — it was already ~12 days stale before
  this session and this session could not update it.

  *Why not:* the Artifact tool refuses a publish to an artifact the conversation has not read
  in full, and this page is ~494 KB / 5,799 lines of generated HTML. Reading it costs more
  context than the whole task did, and yields nothing — the page is a pure render of the
  contract.

  *Proof it is a pure render, so nothing is lost by regenerating:* re-render at the live
  artifact's own commit and diff against it.
  ```
  git worktree add /tmp/wt d3004f1
  (cd /tmp/wt && python3 scripts/render_dashboard.py --artifact /tmp/regen.html)
  diff <live copy> /tmp/regen.html      # 18 lines, ALL build stamps
  ```
  Every difference was a build stamp (the doctype wrapper the publish adds, the MBSE digest,
  the rev, the contract digest, `updated_by`). No viewer has ever saved content into that
  page, so there is nothing to merge.

  *To fix it, in one step:*
  ```
  python3 scripts/render_mbse.py && python3 scripts/render_dashboard.py
  python3 scripts/render_dashboard.py --artifact /tmp/dash.html
  # then the Artifact tool with url = the artifact_url stored in project_state.json
  ```
  A session that has published that artifact before does not hit the read gate. Until this is
  done, **the published board understates the project by two weeks** — including the retraction
  of the "vertical error is delivered by the dash" finding (ADR-0099 second addendum). Treat
  the repo, not the hosted page, as current.
