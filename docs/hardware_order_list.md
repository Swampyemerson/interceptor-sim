...
- **[SETTLED 2026-07-20 — Matek] Target FC** *(SUPERSEDED same-day at checkout: the "one-purchase alt" WON — the ORDERED cart carries the **Holybro Kakute H7 V1.5 + Tekko32 F4 65A** (§0d ORDERED banner + `bom_tiers`; ArduPilot firmware target `KakuteH7`, configs/target_kakute/). The Matek analysis below is kept for the record — ledger `target-fc-matek-vs-kakute`.)*: the SpeedyBee path is dead — F405 V4 DISCONTINUED, F405 V5 OUT OF STOCK everywhere reachable (only a 3× Amazon markup). Switched to **Matek H743 (SLIM/WLITE) + a 45–55 A 6S 4-in-1 ESC** — the ArduPilot-native brand, long-standing OFFICIAL support (not a fresh merge), broadly stocked (GetFPV/Pyrodrone/RDQ/Amazon). Cheaper if in stock: a current **Matek F405** board + ESC; one-purchase alt: **Holybro Kakute H7 + Tekko32**. ~$138–155 (H743) / ~$80 (F405) — +$35–75 vs the SpeedyBee stack; standalone FC so add the ESC. (E)
...
## 0d. STAGED BUY — the order plan (2026-07-17, reconciled with the dashboard `build_plan` + pre-order stress-test)

Order in TWO tiers, gated so you never spend on the wrong thing. **Canonical/always-current: the dashboard's `bom_tiers` + the P0–P5 `build_plan` in `docs/project_state.json` (hosted view: the standing Artifact URL in `artifact_url`).** This section is the order-list mirror.

> **✅ [ORDERED 2026-07-20 — Fable GO-WITH-FIXES]** Cart placed: the **target-drone stack** (Kakute H7 V1.5 + Tekko32 F4 Metal 65A + ECOII 2207×4 + Source One V6 + M10Q + RP1 RX + Pocket M2 TX + CNHL 6S + charger + props), the **seeker kit** (innomaker OV9281 + Pi 5 + Wonrabai 22→15 CSI cable + microSD + reader + PSU), and the **interceptor BRAIN** (Pixhawk 6C Mini + PM02 V3 + Holybro M10 — front-loaded past the money gate; acceptable, ADR-lite deviation logged). **Two fixes before/at checkout:** (1) re-source the **Pi 5 ($175→~$80)** + **SanDisk 128GB ($45→~$20)** from PiShop/CanaKit — recovers ~$120; (2) add the **build/field layer** the cart lacks (solder+safety+XT60, a **microSD for the Kakute** = the target's DataFlash log, 18650s for the TX, +2 LiPo packs, active cooler, tripod, USB-C PD power bank, AprilTag placard, checkerboard, FAA/Remote-ID). Full list + prices in the dashboard `bom_tiers` Tier-1 must-add rows.

**TIER 1 — first steps: NN-validation + compute de-risk (buy FIRST, no result-gate).** Prove the riskiest bets cheaply before any interceptor money. Contents: **target drone (E) + AprilTag placard (E) + OV9281 camera (C) + Pi 5 8GB (C) + checkerboard (C) + USB/power bits (D)**. Hardware ~$475; to actually *fly* the target for the test you also need the ground/safety gear (TX/controller + charger + LiPos + safety/solder, from F) → **~$620–780 realistic first-outdoor-test outlay** if none owned (the ~$310 tripod-test *core* = target + tag + camera). **Do the P0 desk checklist first** (frame-top sweep, placard-sizing, camera paper-checks incl. the vertical-FoV 800-vs-960 check, FAA/Remote-ID). Purpose: the tripod afternoon (build_plan P2) yields TWO curves in one session.

**⛔ THE MONEY GATE (two curves, one test) — this is the money logic, get it right:**
...
| 3D-printed up-tilt + **prop-clearance** camera mount (up-tilt 10–30°, detent ~15°; camera fwd/up of the prop disc) | 1 | $0–15 | $0–15 | Self-print, or JLCPCB/Craftcloud print service | ~~Sets the sim's up15 tilt (ADR-0067)~~ **[UPDATED 2026-07-17 — a FIXED up-tilt is now the FINALIZED decision (builder chose fixed over adaptive for simplicity; adaptive REJECTED). Angle = the airframe's steady DASH PITCH, set from the first real dash ULog (build-specific ~25–40°, NOT sim-sized) — so print an ADJUSTABLE 10–30° bracket and LOCK it after that measurement. Prior up15 NULL (ADR-0068) was a cue-era terminal-PARITY test, a different context. See the pointing decision in `docs/project_state.json`.]**. Bracket retained for PROP-CLEARANCE (§0②, still valid — add #18h): pushes the camera **forward/above the prop disc so the blades sit outside the ~100° FoV** — the $0 fix for the own-prop phantom (§0②). **Run the prop-clearance geometry check before printing final.** PETG/ABS for outdoor heat. $12 buy-bracket if no printer (verify it clears props at 100°). |
...
| Item | Qty | ~Unit | ~Line | Where | Why + key compatibility |
|---|---|---|---|---|---|
| **Matek H743** (SLIM/WLITE) FC + **45–55 A 6S 4-in-1 ESC** (flash ArduCopter) *(SUPERSEDED at checkout — the ORDERED FC is the Holybro Kakute H7 V1.5 + Tekko32, §0d/`bom_tiers`; ledger `target-fc-matek-vs-kakute`)* | 1 | $96–113 + ~$42 | ~$138–155 | GetFPV / Pyrodrone / RDQ / Amazon (WLITE) | **SETTLED here after the SpeedyBee dead-ends** (V4 DISCONTINUED, V5 out of stock everywhere reachable + a 3× Amazon markup — 2026-07-20). Matek = the ArduPilot-native brand with LONG-STANDING official support (not a fresh merge) and broad stock. H743 = H7, onboard SD/flash for the DataFlash log field_score needs, plenty of UARTs. **Standalone FC → add a 4-in-1 ESC** (a bit more wiring than a stack). CHEAPER if in stock: a current **Matek F405** board (~$35–45, minimal ArduPilot build, plenty for a mover) + the same ESC (~$80). One-purchase alt: **Holybro Kakute H7 + Tekko32** (~$110–130, same vendor family as the interceptor's Pixhawk). Runs ArduPilot, not PX4 — fine, it's just a mover. 3–6S, RC via SBUS, GPS via UART. |
