...
**Sources retired 2026-07-10 (ADR-0066).** The four source documents listed
above were deleted from the working tree and retired to git history — recover
any of them with `git show e6f06d3:docs/<filename>`. Portfolio/interview
material (including the other five docs some Evidence cells below still cite
by line number — `interviewer_prep.md`, `portfolio_bullets.md`,
`portfolio_visuals.md`, `WRITEUP.md`, `review_track_fix_delta_20260709.md`,
also retired at the same hash) will be regenerated by a dedicated agent at
project end. **This tracker is now the sole canonical ledger** — it does not
depend on any of the nine retired docs remaining on disk; the Evidence cells
that still cite line numbers in them are historical verification snapshots,
each flagged inline with the same recovery instruction.
...
**7 → TRACKED** (now hold docs/next.md queue slots), **3 → PARTIAL** (real work
...
| DEEP-P9 | Header meta-claim "Nothing here overclaims" is itself an overclaim. | nit | **FIXED** | `94198e8` (2026-07-10 refresh): header now reads "Written not to overclaim — ... judge that intent against the sources, not this sentence," and points at this tracker. |
| DEEP-P10 | Superseded ADR-0030 degraded-cue numbers (9 m/s→1.19, 12 m/s→1.48) still circulated as *current* on some surfaces. | MEDIUM | **FIXED** | `7dad15e`. Confirmed live (at the time): `docs/portfolio_bullets.md:159-160` now explicitly reads "...the earlier ~1.2–1.5 m ADR-0030 figures were flown under the old, too-steep range-noise curve and are superseded." (`docs/portfolio_bullets.md` retired 2026-07-10, ADR-0066; verify via `git show e6f06d3:docs/portfolio_bullets.md`.) |
...
| FWD-2 | v3 span-sidecar footgun: a missing calib sidecar silently falls back to span=1.0, ~8.5% range error, with nothing to catch it. | MEDIUM | **FIXED** | Same pre-registration package (`f6427af`). ADR-0061 (`615ef4c`): "recalibrate the v3 span sidecar FIRST (fit 0.9382), assert `MARKERLESS_SPAN_M` unset, fresh timestamped out." |
...
| FWD-6 | ADR-language precision cluster: grid val-split must never be framed as symmetric generalization; disclose Phase-0 forensics ran on 2 of the 6 gate flights; results are conditional on one nondeterministic render draw; the n=10 sign test is near-unpowerable. | LOW | **FIXED** | ADR-0061 (`615ef4c`) text matches nearly verbatim: "Grid 3/18 m = v3's OWN val split... v2 zero-shot → asymmetric, never framed as symmetric generalization. Phase-0 forensics ran on evalv3_01/02 = TWO of the six gate flights (disclosed)... conditional on this one ADR-0057-nondeterminism render draw; the n=10 flight sign test is near-unpowerable and is not leaned on." |
...
| FWD-8 | Any ADR-0059-CLOSED verdict must be scoped to speed-12/gate-8 (at low target speed the frozen cue stays inside the gate and the fail-closed witness silently fails to bite). | MEDIUM | **FIXED** | Both `check_jam_mc.py`'s SCOPE banner (`f6427af`) and ADR-0059's RESULTS section (`615ef4c`) explicitly scope to "speed-12 / gate-8 ONLY." |
...
| FWD-B3 | Cold-start launch readiness: ground-standby/launch-on-detect gives a 5-13 s window, but cold GPS lock + EKF align + arming is 30+ s — no readiness variable exists anywhere in the model. | MEDIUM | **UNTRACKED** | Not found in the design review doc or docs/next.md. |
...
| FWD-B5 | Cue link has no message authentication — bare UDP JSON straight into `json.loads`, and this is the truth reference that arms handoff. HMAC+freshness is a $0 fix (one dataclass) but touches `m4_intercept.py`, which was held pending jam validation. | MEDIUM (honesty-adjacent) | **TRACKED** | docs/next.md sim-free lane holds the slot ("FWD-B5 HMAC cue-auth ADR draft (design only)" under audit-backlog grooming). The jam-fix commit has landed, so the design half is unblocked; the implementation touches `m4_intercept.py` and stays m4-window-gated. No draft yet. |
...
| FWD-C4 | G5 vertical probe (±1-2 m/s mover altitude schedule, 8 flights, 3D miss) should be pinned into #32's arm list — still unowned. | MEDIUM | **TRACKED** | Confirmed at the 2026-07-10 refresh: docs/next.md sim-queue slot 6 reads "#32 r_hat honesty campaign — aspect/bias/freeze probes + the G5 vertical probe (FWD-C4) pinned into its arm list." |
| FWD-C5 | G8 clutter/decoy-world variant has no task number — the adopted config's gate radii have only ever seen one empty world. | HIGH (per design review's own Tier-2) | **TRACKED** | Same underlying gap as DEEP-P7 — now holds docs/next.md sim-queue slot 8 (task # still to be assigned). |
...
| P-commitsC | `README.md:617/735` and `docs/progress.md:19` still frame the onboard seeker retrain as "in progress" / "the demo should fly the best detector" — ADR-0061 has since settled this (v2 stays deployed; v3 is a NULL). | MEDIUM | **FIXED** | `a0ececf` (README: the T25 demo flies the deployed v2 detector; v3 closed NULL) + `117f973` (docs/progress.md v3 row closed as an honest NULL). |
| P-commitsD | Breakoff-deadband/border-reject + `frozen_vworld` fallback were correctly deferred (they touch held files mid-jam-validation) but the sequencing decision is recorded nowhere — a one-line docs/next.md note was recommended. | LOW | **FIXED** (overtaken by events) | The deferral this asked to document ended: the deferred breakoff-deadband/border-reject work LANDED in `706bb46`/ADR-0062 (which records the sequencing itself), so there is no pending sequencing decision left to note. The `frozen_vworld` fallback remains open separately as DEEP-C2. |
...
is down to **27**. Items that gained a docs/next.md queue slot or task # moved to
...
(docs/next.md queue slots) — DEEP-H2, DEEP-P7/FWD-C5 (G8, sim-queue 8), FWD-B1,
...
| **TRACKED** | 11 *(was 4)* | Has a task # or a docs/next.md queue slot, no code/doc yet. |
...
**Count-verification method note (2026-07-10).** A prior automated recount flagged
that this summary might not match the actual table rows (naive-regex estimate
~30 FIXED / ~22 UNTRACKED / ~70 total). Re-counted by hand, one table at a time,
tallying the literal bolded Disposition value per row (compound labels like
DEEP-C1's `**FIXED (partial)**` counted under their leading word, i.e. FIXED —
same bucket the existing table already used for it, confirmed by cross-checking
against the named PARTIAL/TRACKED membership lists in §4):
- §1 (deep): 1a 5 rows + 1b 22 rows = **27** — FIXED 16, TRACKED 2, UNTRACKED 4, ACCEPTED 3, PARTIAL 2.
- §2 (forward): 2a 8 + 2b 6 + 2c 6 + 2d 8 = **28** — FIXED 11, TRACKED 6, UNTRACKED 10, PARTIAL 1.
- §3 (pipeline/frames): 3a 4 + 3b 12 + 3c 5 + 3d 5 = **26** — FIXED 9, TRACKED 3, UNTRACKED 13, PARTIAL 1.
- Grand total: 27 + 28 + 26 = **81** rows; by disposition FIXED 16+11+9=**36**,
  TRACKED 2+6+3=**11**, UNTRACKED 4+10+13=**27**, ACCEPTED 3+0+0=**3**,
  PARTIAL 2+1+1=**4**; sum 36+11+27+3+4=**81**, matching row count exactly.
- Cross-checked against §4's own named lists — the 4 PARTIAL rows named there
  (DEEP-R2, DEEP-T1, FWD-C2, P-H2), the 11 TRACKED IDs, and the 27 UNTRACKED IDs
  grouped there — all tally to the same counts independently.
- **Verdict: the table above was already correct.** The prior automated
  estimate undercounted because a naive regex on `**FIXED**`/`**PARTIAL**` as
  exact tokens misses compound cells (`**FIXED (partial)**` doesn't match a
  bare `**FIXED**` pattern) and rows that carry two bolded phrases in one cell
  (e.g. DEEP-H6's `**FIXED** ... **result correctly stays HELD**`) — no rows
  were actually re-classified here, only re-verified. Also worth pinning down:
  "81" is **not** a raw pre-consolidation count sitting above a smaller
  post-merge table total — per the intro's own arithmetic (deep=27 +
  forward=28 + pipeline/frames=26 = 81, with the one DEEP-H5/FWD-A3
  cross-audit duplicate *already* folded into that 27), 81 **is** the actual,
  current table-row total; that interpretation is confirmed by this recount,
  not merely asserted.
