...
CONCEPT ("pursuit"): arrive slowly instead of crossing fast.
  ...
  (nominal, guidance-side) camera mount tilt, and closes on the estimate.
  Inside `hold_range_m` the last command is held ("fly through") rather than
  chasing a vanishing/sign-flipping line-of-sight direction. A dropout longer
  than `fallback_s` reverts to a fresh Phase A built from the KF's last
  estimate.
...
class PursuitConfig:
    ...
    v_close_ms: float = 4.0          # m/s, closing speed added along the LOS
    kp_cross: float = 1.5            # 1/s, cross-track correction gain
...
class PursuitRendezvousGuidance:
    ...
    def step(self, t: float, own: VehicleState, det: Optional[Detection]) -> VelCmd:
        ...
        if self._phase == "B":
            ...
            if t - self._last_decode_t > self.cfg.fallback_s:
                self._track = _BeliefTrack(self._kf.pos, self._kf.vel, t)
                self._phase = "A"
    ...
    def _phase_b_cmd(self, own_pos: np.ndarray) -> Tuple[np.ndarray, float]:
        """v_cmd = v_target_est + v_close*unit(r_est) + kp_cross*(cross-track
        component of r_est). "Cross-track" is read relative to the TARGET's
        own estimated track direction (unit(kf_vel)) -- the spec's "r_est
        minus its along-LOS part" is literally the zero vector if "LOS" means
        unit(r_est) itself (that IS r_est's own direction), so this reading
        (perpendicular to the target's track, mirroring Phase A's own
        along-track/cross-track split) is the sensible non-degenerate one;
        flagged as a judgment call in the task's final report."""
        cfg = self.cfg
        ...
        track_dir = _unit(kf_vel, los_dir)
        cross = r_est - float(np.dot(r_est, track_dir)) * track_dir
        cmd_v = kf_vel + cfg.v_close_ms * los_dir + cfg.kp_cross * cross
    ...
    def _measurement_r(self, det: Detection, dir_ned: np.ndarray) -> np.ndarray:
        ...
        cross_sigma = r * cfg.sigma_px / cfg.fx_px
    ...
    def _start_phase_b(self, t: float) -> None:
        pts = self._decode_positions
        pos0 = pts[-1][1]
        if len(pts) >= 2 and (pts[-1][0] - pts[0][0]) > 1e-3:
            vel0 = (pts[-1][1] - pts[0][1]) / (pts[-1][0] - pts[0][0])
        else:
            _, vel0 = self._track.at(t)
        self._kf.init(pos0, vel0, self.cfg.kf_p0_pos_m, self.cfg.kf_p0_vel_ms)
        self._last_decode_t = pts[-1][0]
        self._phase = "B"
