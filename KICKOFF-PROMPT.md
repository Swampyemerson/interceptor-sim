...
are already in this repo and auto-loaded: read **`GOALS.md`** (what we're building
...
- **Checkpoint as you go:** `git commit` at every milestone; keep `CLAUDE.md`,
  `NEXT.md`, and `PROGRESS.md` current so a fresh session resumes cleanly.
...
**Milestones (in order — persist these into `NEXT.md`/`phases/` as you go):**
