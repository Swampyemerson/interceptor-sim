...
**KEY FINDINGS (overnight → THE PIVOT, ~08:00):** The realistic quad's honest Pk is **41% = l2r 81-88% / r2l 0-12%** (audit-CONFIRMED solid, master-seed 42). I spent the night attacking the r2l 0% as a PERCEPTION phantom (the NN locks a big camera-left false box reading ~1.5 m). Four perception levers all failed: range-gate (NULL), `--track` (retracted — geometry artifact), `--fuse-midcourse` (NULL), negative-mining (regressed). **BUT the mining VALIDATION was pivotal:** the hardened seeker SUPPRESSED the phantom (acquires the real r2l target early at ~15-18 m vs the phantom's 1.5 m) **yet r2l STILL missed ~7 m — unchanged.** → **THE PHANTOM IS NOT THE r2l CAUSE. r2l is a GUIDANCE/GEOMETRY ASYMMETRY** (the interceptor catches the mirror-symmetric l2r crosser but not r2l, even with a clean detection). Full story: ADR-0076 add #5-#11. **NOW: a workflow (`wf_r2l_guidance`) is hunting the l2r/r2l guidance asymmetry** (sign/frame-convention suspects: strapdown LOS λ=ψ+β, world→NED north=y/east=x, pro-nav sign, yaw setpoint, target orient-to-velocity bank) → candidate fix → sim A/B. This is the REAL close-interception lever. **v2 seeker stays DEPLOYED (l2r 88%); hardened NOT adopted (under-trained epoch-7 + r2l is not perception).** Also fix `mc_batch.sh:532-535` coverage regex (add #9 Risk 4).
