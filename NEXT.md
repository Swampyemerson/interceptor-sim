...
**KEY FINDINGS (overnight):** (a) honest baseline = 41% Pk@2.5 but it's a DIRECTIONAL split — **l2r 81%, r2l 0/16** (ADR-0076 add #5); (b) r2l fails on a big PHANTOM box (~292 px reading 1.7 m at 15 m true, 9× the real target; ADR-0076 add #6); (c) ADR-0077 range-gate = FAILED/NULL (box-width range too noisy — l2r regressed); (d) **BREAKTHROUGH — the r2l fix is `--track` (a FREE config flip, ADR-0076 add #7): with the CSRT detect-then-track config, r2l = 5/6 (83%) vs NN-only 0/16** — the tracker's temporal consistency filters the transient phantom NN-only chases. This REVISES add #2 (that "NN-only wins" was aggregate, not direction-split). A/B RUNNING (`mc_quad_track_ab_*`, n=32 master-seed 42 + `--track`) to confirm on the exact baseline geometry + get `--track`'s l2r number → config = `--track` globally (if l2r holds) or DIRECTION-AWARE (cue knows the crossing dir; NN-only l2r / `--track` r2l). Takes the quad 41%→~80%+ BOTH ways = the close-interception win, no retrain. Negative-mining (682 frames→`quad_dataset_hardened`, retrain PAUSED) is the fallback. Auto-crop dropped (single crop-NN can't bootstrap acquisition at range).
