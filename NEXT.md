...
**KEY FINDINGS (overnight, audit-reviewed):** The realistic quad's honest Pk is **41% Pk@2.5 = l2r 81% / r2l 0%** (n=32, canonical master-seed 42) — an **independent Opus audit CONFIRMED this baseline is solid** (not confounded by geometry/render/load/seeds; miss = honest physical CPA). The r2l 0% is a **PHANTOM**: the NN locks a big camera-left false box reading ~1.5 m (true ~15 m) that triggers the seeker HANDOFF → the interceptor commits to a false close target → CPA stalls ~7.4 m. **Four fix levers tried, all logged (ADR-0076 add #5-#10, ADR-0077):** range-gate = NULL; `--track` = RETRACTED (5/6 was a non-representative easier geometry; 0/9 on canonical — methodology lesson: reproduce the gate geometry); `--fuse-midcourse`-off = NULL (r2l unchanged → phantom acts via handoff, not midcourse fusion). **REMAINING FIX = NEGATIVE-MINING** (the audit initially feared it a NULL since the phantom is universal, but phantom+target are CO-LOCATED yet SIZE-separable → an appearance-trained NN is the correct discriminator; add #10): 1588 canonical-r2l phantoms mined → `quad_dataset_hardened` → **retrain RESUMED (pid in hardened_train_log.txt), validation queued on UNSEEN master-seed 123** (leakage-free, `validate_hardened.sh`). Success = r2l recovers + l2r holds; NULL → escalate to 4K/higher-res retrain (ADR-0074). Also: fix `mc_batch.sh:532-535` coverage regex (CSV coverage/breakoff/first_det columns are buggy — don't cite; add #9 Risk 4).
