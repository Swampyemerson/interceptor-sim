...
**KEY FINDINGS (overnight, corrected):** (a) honest quad baseline = 41% Pk@2.5 = DIRECTIONAL split — **l2r 81%, r2l 0/16** (ADR-0076 add #5); (b) r2l fails on a big PHANTOM box (~292px reading 1.5m at 15m true; add #6); (c) range-gate (ADR-0077) NULL; (d) **`--track` is NOT the fix (add #7 RETRACTED by add #8)** — it appeared to fix r2l (5/6) only on a NON-representative easier geometry (30m target start); on the CANONICAL geometry (master-seed 42, ~14m start) the A/B gave `--track` 0/9 (hurts l2r 1.2→3m via CSRT flicker, still fails r2l). **METHODOLOGY LESSON: reproduce the canonical gate geometry, not a convenient fixed start.** (e) **CURRENT PATH = NEGATIVE-MINING from the CANONICAL r2l geometry** — capture_canonical_mine_retrain.sh RUNNING (flight 1 reproduced the 7.6m failure ✓); mines the actual phantoms → `quad_dataset_hardened` → retrain → validate NN-only+hardened on master-seed-42 (success = r2l recovers toward 81% without breaking l2r). NN-only stays deployed; honest quad = 41% until a VALIDATED fix lands.
