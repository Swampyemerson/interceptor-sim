...
- [x] **Bdash control + anti-mirage 2×2 SETTLED** (`cdf695b`, then CORRECTED `c1baeb9`). Baseline **A-l2r is camera-DRIVEN** (−1.18 m, 4/4 — first confirmed camera-driven 3D-quad intercept); **B-vs-Bdash camera worse 8/8** (Phase A "sub-1m camera-tracked" for the lever REFUTED). **CORRECTION (independent Opus 5 review + validated ballistic model, MAE 0.29 m):** the "closing-speed cost" attribution is **confounded** — the aim constant is mis-sized (`collision_lead_heading` assumes constant speed; the dash accelerates from rest → flown +30° is wrong, optimal ~64° at a=3.57 / ~20° at a=10) AND the cap gave the LEAST centring (+5°). Camera-hurt is **measured** as the ADR-0056 aspect bias (λ−gtLOS +9..+17° l2r / −17..−21° r2l). `antimirage_verdict.py`.
- [x] **Field bring-up kit** (`334d0ca`) — `scripts/field/` (device detect / camera live / AprilTag desk / props-off FC bench / log pull) + `docs/field_bringup.md`; `bringup.sh` exit 0 no-hardware, `selftest.sh` 22 checks pass. Camera is MIPI-CSI to the Pi → `--pi/--local/--replay`. FC bench check is arming-impossible (AST-audited).
- [x] **Flight-plan + launch-mech docs** (`334d0ca`) — `docs/flight_plan_candidates.md` (pre-registered decomposition sweep + kill-criteria + Xdash anti-mirage twins), `docs/launch_mechanism_plan.md` (ground-takeoff→standby-hover→human-trigger→coded-dash, **camera bearing latch on the critical path**), `scripts/experiments/flight_plans/` harness + validated `dash_cpa_model.py`.
- [x] **Detection/tracking method stack** (`8e84ac6`) + **hardware default-weights defect FIXED** (`c3bd0d9`): the Pi-5 deploy seeker + tripod scorer defaulted to the sim-only **quad_v2 (1.1% recall on real imagery)** → re-pointed to the real-data **n-mono (44.2%)**, tripod curve (b) now scores both, per-model grayscale handling; sim path proven **byte-identical**, 177 tests pass.

**🎯 CORRECTED ROADMAP — the decomposition sweep is the active sim work (fly-first order):**
1. **G20dash** (sim, `bb65omhm1`, FLYING) — bias 20, dash-only, level. Isolates the aim-mis-sizing vs Adash (bias 30). CONFIRM iff ≥6/8 paired better AND median ≤0.90 m; REFUTE iff worse ≥5/8 or median ≥1.37 m. Model predicts ~0.14 m.
2. **C / Cdash** (loft-only, bias 30, up10 shadow) — isolates the cap. ADOPT iff 8–12 m REAL recall ≥15% AND <5 m recall ≥50% AND handoff speed ≥4.0 m/s AND miss not worse than A on ≥6/8.
3. **G20** (camera live, bias 20) gated on G20dash; then **E/Edash** (bias 64, cap-optimal aim); then **CG**; then **seed 777**. Adoption into the contract needs pooled **≥13/16** paired (6/8 is NOT significant).
- ⛔ **Phase B FOV-hold is DEMOTED — the WRONG lever** (Arm B was centred −3.6° and still lost; the failure is bearing BIAS, not FOV loss). The real lever is **aspect-bias compensation** (`--terminal-bearing-bias-deg`, code-gated, ~unbuilt). `flight/fov_guidance.py` stays parked.

**🔧 TRACKED DEFECTS / DECISIONS (deferred, need a SITL slot or an ADR call — NOT rushed):**
- [ ] **`seeker_loop.py` connect() hang** (worker-2 found): `await drone.connect()` can block forever on a dead link (mavsdk_server delays its gRPC port until vehicle discovery) BEFORE the `_CONNECT_TIMEOUT_S` guard at L694 fires → a mis-wired props-off bench hangs instead of failing clean. Fix = wrap `connect()` in a timeout + kill the server child. **Untestable without SITL/an FC → do it in a dedicated SITL slot, verify via `check_deploy_sitl.sh`, then commit.** (Field kit's own `fc_link_check.py` already wraps this, so Emerson's bench check is safe meanwhile.)
- [ ] **Money-gate ADR-lite** (worker-6 flagged): `tripod_score.py gate_verdict` uses a mean-rate streak-burn model that is **optimistic vs consecutive-run-length by up to 9.25× at p=0.44** — always in the dangerous direction for the ~$740 purchase gate. No verdict is published yet (tripod is weeks out), so this is the SAFE time to switch it to the conservative run-length model. Decide + apply before the tripod day. (Currently documented as a TODO in the gate docstring so no reader is misled.)
- [ ] **BOM correction:** the interceptor AIRFRAME (GEPRC Mark5 / TBS Source One V5) is **NOT ordered** — it's Tier-2, gated on the tripod AprilTag curve. Ordered = target stack (Source One V6) + seeker kit + interceptor BRAIN (6C Mini + PM02 + M10). The launch-mech/field docs split "now" vs [T2]; keep this straight everywhere.
