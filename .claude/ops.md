# Operating model — imported by CLAUDE.md (@.claude/ops.md)

The session-facing summary lives in CLAUDE.md; this file holds the full
operating model. The builder is new to simulation and guidance and often
reads project surfaces on a phone — plain English, jargon glossed inline.
...
`docs/next.md`, `docs/progress.md`, and the ADR log are SUBORDINATE views that must stay consistent
with it (the drift check + the contradiction ledger enforce it).

- **ORIENT (session start):** read `project_state.json` FIRST — before `docs/next.md` or the ADRs —
...
- **A fix is not done until its EFFECT is observed end-to-end, and drift is swept the
  ...
  docs/next.md, README, any T25/demo copy — not just the stage note; the drift-check is
...
You run in **WSL2 (Ubuntu 24.04)** with an **NVIDIA GPU available** (`nvidia-smi` works; libs under `/usr/lib/wsl/lib`). Gazebo can render **GPU-accelerated**, and WSLg gives a display for the GUI. Still prefer **headless** (`HEADLESS=1`) for automated/batch runs (faster + reproducible); use the GPU for camera rendering and the final demo. PX4 and MAVSDK both run here, so connect over local UDP (`udpin://0.0.0.0:14540`).
