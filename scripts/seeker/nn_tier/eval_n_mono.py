#!/usr/bin/env python3
"""EVALUATE phase — honest held-out-source eval of the nn_tier `n-mono` fine-tune.

WHAT THIS IS (nn_tier EVALUATE, 2026-07-21)
-------------------------------------------
Scores the new markerless drone detector (`weights/nn_tier/n-mono.onnx`,
COCO-init yolo11n @640 fine-tuned on the GRAYSCALE nn_tier corpus) the honest
way, against the deployed sim-trained net and the MIT real-photo candidates:

  * Eval set = the Dataset phase's SOURCE-DISJOINT TEST split
    (scripts/seeker/data/nn_tier/gray images/test + labels/test +
    manifest_test.csv). dut = a whole NEVER-SEEN source; nps = held-out
    FLIGHT clips; plates = held-out photographers' desert negatives.
    The model never trained on any frame from any of these sources
    (ADR-0061 anti-mirage rule: split by source/video/scene, never frame).
  * GRAYSCALE only — the deployment modality (OV9281 is mono;
    camera_paper_check.md item 4). The gray test images are grayscale on
    disk; clutter frames are converted BGR->GRAY->3ch.
  * Metrics: AP50 (all-point), precision/recall @ the deployed operating
    point conf=0.25 (v3_onnx_infer.DEPLOY_CONF), false-fire density on
    drone-free images (overall + per source), CPU ms/frame.
  * RECALL BY SCALE (px @1280-equivalent, the 6-20 m terminal-band proxy:
    viewpoint_and_deploy_spec.md A3 puts the 5" target at ~7-36 px @1280
    across 6-20 m, i.e. the <=40 px bins) and BY FRAME POSITION, the latter
    scored through the fixed gate `box_scoring.box_hits_gt` (READ-ONLY
    import; the ADR-0076 #18k centred/size gate) with declared approximate
    intrinsics for real imagery (see POSITION GATE NOTE below).
  * CLUTTER false-fire: the transfer-bet drone-free segments
    (docs/transfer_bet_kill_test.md; neg_bird/neg_seagull/neg_sky/
    neg_ground processed frames) — real desert-ish clutter, small n
    (17 frames), reported per segment with n.

POSITION GATE NOTE. box_hits_gt was built for sim captures with known
intrinsics (FX 539.936 @1280x960). Real test images have UNKNOWN intrinsics,
so the off-axis sec^2 widening uses a declared proxy: fx=fy=FX*(W/1280),
cx=W/2, cy=H/2, gt_scale=1.0 (labels ARE the truth extent — no re-scale),
tol=15*(W/1280), centre_lag_px=0. The gate here is a HIT-DEFINITION for the
position breakdown (centre-in + 3x size guard), reported alongside the
IoU-0.5 metrics, not a replacement for them.

HONESTY / anti-mirage (CLAUDE.md, ADR-0061):
  * Frame metrics do NOT certify a seeker — flight/tripod evidence does.
    This ranks the fine-tune against its baselines on held-out real sources.
  * Slow MIT models run on a DETERMINISTIC stratified subsample (seed 0,
    n disclosed per row) — full-split rows are marked subset=full.
  * Labels score only; nothing here feeds a live seeker.

Run (CPU, read-only seeker venv):
  .venv-seeker/bin/python scripts/seeker/nn_tier/eval_n_mono.py \
      --models n_mono v2_deployed --tag <tag>
Self-test (tiny 6-image set through the first available model; exits 0/1):
  .venv-seeker/bin/python scripts/seeker/nn_tier/eval_n_mono.py --self-test
"""
...
from baseline_eval import (run_model_on_image, to_gray3, ap50)      # noqa: E402
import box_scoring                                                  # noqa: E402

WEIGHTS = os.path.join(SEEKER, "weights")
DATA_ROOT = os.path.join(SEEKER, "data", "nn_tier")
CLUTTER_ROOT = os.path.join(SEEKER, "data", "transfer_bet_footage")
...
CLUTTER_SEGS = ["neg_bird", "neg_seagull", "neg_sky", "neg_ground"]
...
def gate_hit(dets25, gcx, gcy, gw, gh, W, H):
    ...
    for real imagery (see module docstring POSITION GATE NOTE)."""
    s = W / 1280.0
...
def eval_model_on_rows(model_key, rows, progress_fh=None, tag=""):
    ...
    for k, r in enumerate(rows):
        ip = os.path.join(DATA_ROOT, r["gray_img"])
        frame = cv2.imread(ip)
        if frame is None:
            continue
        H, W = frame.shape[:2]
        lp = os.path.join(DATA_ROOT, r["gray_img"]
...
def eval_clutter(model_key, progress_fh=None):
    """Transfer-bet drone-free segments, GRAY. Returns per-segment fire stats."""
    path, drone_cls, n_classes = MODELS[model_key]
    sess, iname, imgsz = load_session(path, intra_threads=8)
    out = {}
    for seg in CLUTTER_SEGS:
        frames = sorted(f for f in glob.glob(os.path.join(CLUTTER_ROOT, seg, "*"))
                        if not os.path.basename(f).startswith("_raw_"))
        n_fired = n_fires = n_ok = 0
        for fp_ in frames:
            ...
            n_fired += 1 if d25 else 0
            n_fires += len(d25)
        out[seg] = {"n": n_ok, "fired": n_fired, "fires": n_fires,
                    "fire_rate": round(n_fired / max(n_ok, 1), 3)}
        if progress_fh:
            progress_fh.write(f"clutter {model_key} {seg}: {out[seg]}\n")
            progress_fh.flush()
    tot_n = sum(v["n"] for v in out.values())
    tot_f = sum(v["fired"] for v in out.values())
    out["ALL"] = {"n": tot_n, "fired": tot_f,
                  "fires": sum(v["fires"] for v in out.values()),
                  "fire_rate": round(tot_f / max(tot_n, 1), 3)}
    return out
...
def self_test():
    ...
    available model (n_mono if exported, else the deployed v2). Exits 0/1."""
    ok = True
    # match_boxes: 1 gt; det hits @.9 + far fp @.3 -> P/R@25 both computable.
    gts = [(50, 50, 20, 20)]
    ...
    if (pos_bin_of(640, 480, 1280, 960) != POS_BINS[0]
        ...
        ok = False
    # the fixed gate: centred det on centred gt must HIT; giant blob must MISS.
    ...
    if gate_hit([{"cx": 640, "cy": 480, "w": 376, "h": 368, "score": .9}],
        ...
        ok = False
    # tiny end-to-end eval: 6 real test rows through the first available model.
    model = next((m for m in ("n_mono", "v2_deployed")
    ...
    if model is None:
        ...
        if s["subset_n"] != 6 or len(pi) != 6 or not (0 <= s["recall@25"] <= 1):
            ...
            print(f"tiny eval OK on {model}: {json.dumps(s)}")
...
def main():
    ...
    ap.add_argument("--models", nargs="+", default=["n_mono", "v2_deployed"])
    ...
    ap.add_argument("--clutter", action="store_true",
                    help="also run the transfer-bet drone-free clutter segments")
    ...
    prog = open(os.path.join(OUT_DIR, f"eval_n-mono_progress_{tag}.log"), "a")
