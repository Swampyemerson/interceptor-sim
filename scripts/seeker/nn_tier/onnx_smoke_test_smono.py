#!/usr/bin/env python3
"""Self-test for the nn_tier s-mono ONNX export (the yolo11s "can we afford
bigger" arm): load the model, run ONE real inference (a real test-split
grayscale frame from the nn_tier corpus, letterboxed to the model's input
size) and sanity-check the output tensor (finite, right shape). Exit 0 =
model loads and runs; exit 1 = anything wrong. Run in .venv-seeker (has
onnxruntime + cv2 -- NOT a training env).

Sibling of onnx_smoke_test.py (n-mono) -- distinct filename, points at
s-mono.onnx instead, otherwise identical logic. This is a LOAD+RUN smoke
test only, not an accuracy gate -- it proves the exported ONNX is
well-formed and executable, matching the deliverable's self_test_cmd
contract. Frame-level accuracy is reported separately from results.csv
(flagged as NOT the deployment gate per the honesty rules).
"""
...
WEIGHTS = os.path.join(REPO, "scripts", "seeker", "weights", "nn_tier", "s-mono.onnx")
