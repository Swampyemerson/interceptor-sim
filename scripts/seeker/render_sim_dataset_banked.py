...
BANKED VARIANT (2026-07-11): identical to render_sim_dataset.py except `teleport()`
now also commands ROLL (bank) + PITCH via a full aircraft-frame ZYX-Euler quaternion
(`--banks-deg`, `--pitches-deg`), instead of yaw-only. WHY: the quad seeker trained on
a level+yaw-only grid (quad_dataset) detected `fpv_quad_enemy` at 23 m but lost lock
during the terminal dash once the target's `--orient-to-velocity` banks (up to 55 deg)
and pitches nose-down (up to 25 deg) -- aspects the detector never saw. The
`_euler_zyx_to_quat` formula here is copied verbatim from `scripts/m4_target_mover.py`
so the captured poses match exactly what the live mover commands (fpv_quad_enemy's
nose is baked +X, so unlike fpv_target_markerless there is no extra
model_forward_offset to compose). HONESTY: the gt label still comes from
`PoseTracker.ground_truth_rel_optical()`, which uses only the target's WORLD POSITION
(not its orientation) projected through the known body extent -- the box is a
bounding-sphere approximation, so it is ORIENTATION-INVARIANT and auto-labels
banked/pitched frames exactly as correctly as level ones. No orientation ever reaches
inference; this is training-data auto-labeling only (ADR-0033 B / CLAUDE.md gt_*
boundary).

WHY (original, still true): off-the-shelf detectors (COCO YOLOv8n, the MIT drone YOLO11x) do NOT transfer
...
def _euler_zyx_to_quat(roll, pitch, yaw):
    ...
    +X-forward) -> world orientation quaternion (w, x, y, z). Copied verbatim from
    scripts/m4_target_mover.py so captured poses match what --orient-to-velocity
    actually commands in the live sim (kept in sync deliberately, not re-derived)."""
...
def teleport(x, y, z, yaw=0.0, roll=0.0, pitch=0.0, timeout_s=4.0) -> bool:
    # aircraft-frame ZYX quaternion: roll (bank) + pitch (nose-up/down) + yaw.
    # roll=pitch=0 reduces exactly to the parent script's yaw-only qz,qw --
    # verified numerically at retrain time (asserted for yaw in
    # {0,+-25,+-90,180,37.5} deg, plus a cross-check against
    # m4_target_mover._euler_zyx_to_quat for banked+pitched+yawed cases; both PASS).
...
def main() -> int:
    ...
    ap.add_argument("--banks-deg", default="0",
                    help="target ROLL/bank variations (deg) for aspect diversity "
                         "(BANKED variant; default 0 = level, matches the parent script)")
    ap.add_argument("--pitches-deg", default="0",
                    help="target PITCH variations (deg, +=nose-down in this ZYX "
                         "convention) for aspect diversity (BANKED variant; default "
                         "0 = level, matches the parent script)")
