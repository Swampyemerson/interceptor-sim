...
across this retrain are the dataset (quad_dataset_crop, LEVEL union BANKED) and
...
training set never showed. quad_dataset_crop (merge_quad_v2.py) adds those
...
weights/drone_finetuned_quad_crop.{pt,onnx} and prints the completion sentinel
CROP_TRAIN_EXPORT_DONE.
...
RUN_DIR = os.path.join("scripts", "seeker", "runs", "drone_finetune_quad_crop")
LAST = os.path.join(RUN_DIR, "weights", "last.pt")
BEST = os.path.join(RUN_DIR, "weights", "best.pt")
DATA = os.path.join("scripts", "seeker", "data", "quad_dataset_crop", "data.yaml")
BASE = os.path.join(REPO, "yolo11n.pt")
DST_PT = os.path.join("scripts", "seeker", "weights", "drone_finetuned_quad_crop.pt")

TRAIN_KW = dict(data=DATA, imgsz=640, epochs=60, batch=16, seed=0,
                project=os.path.join(HERE, "runs"), name="drone_finetune_quad_crop",
                exist_ok=True)
...
def main() -> int:
    ...
    print(f"CROP_TRAIN_EXPORT_DONE -> {DST_PT} + .onnx", flush=True)
    return 0
