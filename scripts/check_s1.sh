...
kill_stale_sim() {
    echo "[check_s1] Killing any stale px4 / gz sim processes..."
...
teardown_sim() {
    echo "[check_s1] Tearing down sim processes..."
...
if [[ ! -d "$PX4_DIR" ]]; then
    echo "[check_s1] FAIL: PX4-Autopilot not found at $PX4_DIR"
    echo "FAIL"
    exit 1
fi

if [[ ! -x "$VENV_PYTHON" ]]; then
    echo "[check_s1] FAIL: venv python not found at $VENV_PYTHON (run the env setup first)"
    echo "FAIL"
    exit 1
fi

if [[ ! -f "$REPO_WORLD" ]]; then
    echo "[check_s1] FAIL: $REPO_WORLD not found"
    echo "FAIL"
    exit 1
fi
...
if [[ ! -e "$WORLD_SYMLINK" || "$(readlink -f "$WORLD_SYMLINK" 2>/dev/null)" != "$(readlink -f "$REPO_WORLD")" ]]; then
    echo "[check_s1] Linking $WORLD_SYMLINK -> $REPO_WORLD"
...
for LAW in pursuit pronav; do
    echo ""
    echo "[check_s1] ================ Law: $LAW ================"

    kill_stale_sim

    TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"
    SIM_LOG="$LOGS_DIR/check_s1_sim_${LAW}_${TIMESTAMP}.log"
    RUN_LOG="$LOGS_DIR/check_s1_run_${LAW}_${TIMESTAMP}.log"

    echo "[check_s1] Launching HEADLESS PX4 SITL + Gazebo (gz_x500_mono_cam, world=apriltag) for law=$LAW..."
    echo "[check_s1] Sim output -> $SIM_LOG"
    ...
    setsid bash -c "cd '$PX4_DIR' && tail -f /dev/null | env PX4_GZ_WORLD=apriltag GZ_SIM_RESOURCE_PATH='$REPO_ROOT/models' HEADLESS=1 make px4_sitl gz_x500_mono_cam" \
        > "$SIM_LOG" 2>&1 &
    SIM_PID=$!
    echo "[check_s1] Sim launched (pid/pgid $SIM_PID) for law=$LAW."

    echo "[check_s1] Waiting up to ${READY_TIMEOUT_S}s for \"$READY_STRING\" in sim log..."
    ready=0
    elapsed=0
    while (( elapsed < READY_TIMEOUT_S )); do
        ...
        if ! kill -0 "$SIM_PID" 2>/dev/null; then
            echo "[check_s1] FAIL: sim process exited early (pid $SIM_PID) for law=$LAW. See $SIM_LOG"
            echo "FAIL"
            exit 1
        fi
        sleep 2
    ...
    if [[ "$ready" -ne 1 ]]; then
        echo "[check_s1] FAIL: did not see \"$READY_STRING\" within ${READY_TIMEOUT_S}s for law=$LAW. See $SIM_LOG"
        echo "FAIL"
        exit 1
    fi

    echo "[check_s1] Sim ready. Sleeping ${POST_READY_SETTLE_S}s to let the EKF settle before MAVSDK connects..."
    sleep "$POST_READY_SETTLE_S"

    echo "[check_s1] Pre-placing the tag at the engagement start ($TARGET_START_X, $TARGET_START_Y, $TARGET_START_Z)..."
    placed=0
    for attempt in 1 2 3; do
        ...
        echo "[check_s1] WARNING: tag pre-placement attempt $attempt failed, retrying..."
        sleep 1
    done
    if [[ "$placed" -ne 1 ]]; then
        echo "[check_s1] FAIL: could not pre-place the tag after 3 attempts for law=$LAW"
        echo "FAIL"
        exit 1
    fi
    echo "[check_s1] Tag pre-placed."
    ...
    echo "[check_s1] Running m4_intercept.py --law $LAW ${EXTRA_ARGS[*]:-} (outer timeout ${PY_TIMEOUT_S}s)..."
    echo "[check_s1] Run output -> $RUN_LOG (also streamed below)"
    timeout "$PY_TIMEOUT_S" "$VENV_PYTHON" "$REPO_ROOT/scripts/m4_intercept.py" --law "$LAW" --fpv --target-vel "$TARGET_VEL" "${EXTRA_ARGS[@]}" 2>&1 | tee "$RUN_LOG"
    PY_EXIT="${PIPESTATUS[0]}"
    EXITCODE[$LAW]="$PY_EXIT"

    if [[ "$PY_EXIT" -eq 124 ]]; then
        echo "[check_s1] FAIL: m4_intercept.py (law=$LAW) hit the outer ${PY_TIMEOUT_S}s timeout. See $SIM_LOG and $RUN_LOG"
    fi

    RESULT_LINE="$(grep '^M4_RESULT ' "$RUN_LOG" | tail -n1)"
    if [[ -z "$RESULT_LINE" ]]; then
        echo "[check_s1] FAIL: no M4_RESULT line found in output for law=$LAW"
        MISS[$LAW]=""
        CLEAN[$LAW]=""
    else
        ...
        echo "[check_s1] Parsed: law=$LAW miss=${MISS[$LAW]} clean=${CLEAN[$LAW]} engaged=${ENGAGED[$LAW]} (exit $PY_EXIT)"
    fi
...
echo ""
echo "[check_s1] ================ Comparison ================"
echo "[check_s1] pursuit: miss=${MISS[pursuit]:-<missing>} m  clean=${CLEAN[pursuit]:-<missing>}  exit=${EXITCODE[pursuit]:-<missing>}"
echo "[check_s1] pronav : miss=${MISS[pronav]:-<missing>} m  clean=${CLEAN[pronav]:-<missing>}  exit=${EXITCODE[pronav]:-<missing>}"
...
for LAW in pursuit pronav; do
    ...
    if [[ "${ENGAGED[$LAW]:-}" != "1" ]]; then
        echo "[check_s1] FAIL reason: $LAW never reached ENGAGE (engaged=${ENGAGED[$LAW]:-<missing>})."
        PASS=0
    fi
    if ! is_number "${MISS[$LAW]:-}"; then
        echo "[check_s1] FAIL reason: $LAW miss distance not a valid number (${MISS[$LAW]:-<missing>})."
        PASS=0
    fi
done
if [[ "${CLEAN[pronav]:-}" != "1" ]]; then
    echo "[check_s1] FAIL reason: pronav flight not clean (clean=${CLEAN[pronav]:-<missing>})."
    PASS=0
fi

if [[ "$PASS" -eq 1 ]] && ! awk -v m="${MISS[pronav]}" -v g="$PRONAV_MISS_GATE_M" 'BEGIN{exit !(m < g)}'; then
    echo "[check_s1] FAIL reason: pro-nav miss ${MISS[pronav]} m >= ${PRONAV_MISS_GATE_M} m gate."
    PASS=0
fi

if is_number "${MISS[pursuit]:-}" && is_number "${MISS[pronav]:-}"; then
    DELTA="$(awk -v a="${MISS[pursuit]}" -v b="${MISS[pronav]}" 'BEGIN{printf "%.3f", a-b}')"
    echo "[check_s1] delta (pursuit miss - pronav miss) = ${DELTA} m (positive = pro-nav closed tighter, as expected)"
    if awk -v a="${MISS[pursuit]}" -v b="${MISS[pronav]}" 'BEGIN{exit !(a < b)}'; then
        echo "[check_s1] *** WARNING ***: pursuit miss (${MISS[pursuit]} m) is SMALLER than pro-nav's"
        echo "[check_s1] *** WARNING ***: (${MISS[pronav]} m). That is the OPPOSITE of the expected"
        echo "[check_s1] *** WARNING ***: result (pro-nav should beat pursuit against a mover) and"
        echo "[check_s1] *** WARNING ***: needs investigation before this milestone can be honestly"
        echo "[check_s1] *** WARNING ***: closed out, even though the gate below may still pass on"
        echo "[check_s1] *** WARNING ***: pro-nav's own number."
    fi
else
    echo "[check_s1] Could not compare pursuit vs pronav miss numerically (one or both missing/nan)."
fi
