#!/usr/bin/env python3
"""Self-test for the nn_tier n-mono-aug ONNX export (the HEAVY-augmentation
"robustness arm" -- blur/dust/high-desert-lighting Albumentations pipeline,
HSV off since mono, see train_daemon_nn_tier_aug.py): load the model, run ONE
...
Sibling of onnx_smoke_test.py (n-mono) / onnx_smoke_test_smono.py (s-mono) --
distinct filename, points at n-mono-aug.onnx instead, otherwise identical
logic. This is a LOAD+RUN smoke test only, not an accuracy gate -- it proves
the exported ONNX is well-formed and executable, matching the deliverable's
...
WEIGHTS = os.path.join(REPO, "scripts", "seeker", "weights", "nn_tier", "n-mono-aug.onnx")
